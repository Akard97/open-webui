...
describe('LIST_COLUMNS', () => {
	it('lists the six optional columns in fixed order', () => {
		expect(LIST_COLUMNS.map((c) => c.key)).toEqual(['assignee', 'start', 'due', 'priority', 'labels', 'progress']);
	});
});
...
describe('defaultColumnPrefs', () => {
	it('enables every optional column', () => {
		expect(defaultColumnPrefs()).toEqual({
			assignee: true, start: true, due: true, priority: true, labels: true, progress: true
		});
	});
});
...
describe('parseColumnPrefs', () => {
	...
	it('merges a partial object over defaults', () => {
		expect(parseColumnPrefs('{"labels":false,"progress":false}')).toEqual({
			assignee: true, start: true, due: true, priority: true, labels: false, progress: false
		});
	});
...
describe('gridTemplate', () => {
	it('builds name + all columns + menu when everything is visible', () => {
		expect(gridTemplate(defaultColumnPrefs())).toBe(
			'minmax(180px, 1fr) 120px 120px 120px 130px 160px 120px 36px'
		);
	});
	it('omits hidden columns but keeps name and menu', () => {
		expect(gridTemplate({ assignee: true, start: true, due: true, priority: true, labels: false, progress: false })).toBe(
			'minmax(180px, 1fr) 120px 120px 120px 130px 36px'
		);
	});
});
