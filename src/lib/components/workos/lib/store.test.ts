...
vi.mock('./api', () => ({
	...
	getNotificationCounts: vi.fn(async () => ({
		unread: 0, by_type: { assigned: 0, subtask_assigned: 0, mentioned: 0, replied: 0, commented: 0, status_changed: 0 }
	})),
...
describe('inbox notification store', () => {
	beforeEach(() => {
		...
		unreadCount.set(0);
		notificationCounts.set({ unread: 0, by_type: { assigned: 0, subtask_assigned: 0, mentioned: 0, replied: 0, commented: 0, status_changed: 0 } });
	});
	...
	it('applyNotificationEvent prepends and bumps per-type counts', () => {
		...
		expect(get(notificationCounts)).toEqual({
			unread: 1, by_type: { assigned: 0, subtask_assigned: 0, mentioned: 1, replied: 0, commented: 0, status_changed: 0 }
		});
	});

	it('markRead decrements the matching type count', async () => {
		notifications.set([mkN({ id: 'a', type: 'assigned' })]);
		notificationCounts.set({ unread: 1, by_type: { assigned: 1, subtask_assigned: 0, mentioned: 0, replied: 0, commented: 0, status_changed: 0 } });
		await markRead(['a']);
	...
	it('archiveNotificationsAction moves the row out optimistically and marks it read', async () => {
		...
		notificationCounts.set({ unread: 2, by_type: { assigned: 0, subtask_assigned: 0, mentioned: 0, replied: 0, commented: 2, status_changed: 0 } });
	...
	it('archive failure rolls lists and counts back', async () => {
		...
		notificationCounts.set({ unread: 1, by_type: { assigned: 0, subtask_assigned: 0, mentioned: 0, replied: 0, commented: 1, status_changed: 0 } });
		...
		expect(get(notificationCounts)).toEqual({
			unread: 1, by_type: { assigned: 0, subtask_assigned: 0, mentioned: 0, replied: 0, commented: 1, status_changed: 0 }
		});
	});
	...
	it('rollback keeps realtime rows that arrived during the failed request', async () => {
		...
		expect(get(archivedNotifications)).toHaveLength(0);
		expect(get(notificationCounts)).toEqual({
			unread: 2, by_type: { assigned: 1, subtask_assigned: 0, mentioned: 0, replied: 0, commented: 1, status_changed: 0 }
		});
	});
...
describe('inbox rollback isolation + pagination cursor', () => {
	...
	it('markAllRead failure refetches authoritative counts instead of restoring a stale snapshot', async () => {
		...
		vi.mocked(apiMock.getNotificationCounts).mockResolvedValueOnce({
			unread: 3, by_type: { assigned: 2, subtask_assigned: 0, mentioned: 0, replied: 0, commented: 1, status_changed: 0 }
		});
