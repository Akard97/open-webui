...
- Destructive buttons (DECIDED 2026-07-10 — soft tint KEPT): Delete team / Delete workspace / Restrict are now nova soft red-tint pills (`bg-destructive/10 text-destructive`), not the old solid red. Same on comment-delete (hover-reveal red pill). Re-raise only if it reads too weak live.
