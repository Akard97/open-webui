...
- **Reuse** `build_loader_from_config(request)` (`backend/open_webui/retrieval/utils.py`) + `Loader.aload(filename, content_type, file_path)` (`backend/open_webui/retrieval/loaders/main.py`), joining `Document.page_content`. **No `Files` row, no `process_file`, no vector DB** — calling the `Loader` directly never touches the embedding/RAG path.
