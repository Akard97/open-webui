...
async def get_image_base64_from_url(url: str, user=None) -> Optional[str]:
    try:
        if url.startswith('http'):
            ...
            # file-ID resolver, which enforces the ownership/access check.
...
async def get_image_base64_from_file_id(id: str, user=None) -> Optional[str]:
    ...
    # Gate file-by-id resolution by ownership (CVE-2026-54009, fixed upstream
    # in 0.9.6). image_url values are client-supplied: without this check a
    # caller can name another user's file id, and the server reads that file,
    # inlines it base64 into the LLM request, and leaks it back via OCR.
