        ...
        **(
            {
                ...
                'X-Title': 'Osool Intelligence Hub',
            }
    ...
    try:
        ...
        try:
            ...
            raise HTTPException(
                ...
                detail=detail if detail else 'Osool Intelligence Hub: Server Connection Error',
            )
    ...
    if url_idx is None:
        ...
        async with aiohttp.ClientSession(
            ...
            try:
                ...
                raise HTTPException(status_code=500, detail='Osool Intelligence Hub: Server Connection Error')
            except Exception as e:
...
def _normalize_stored_item(item: dict) -> dict:
    ...
    Osool Intelligence Hub stores extra bookkeeping fields (``id``, ``status``,
...
async def proxy(path: str, request: Request, user=Depends(get_verified_user)):
    ...
    try:
        ...
        raise HTTPException(
            status_code=r.status if r else 500,
            detail='Osool Intelligence Hub: Server Connection Error',
        )
    finally:
        if not streaming:
            await cleanup_response(r)
